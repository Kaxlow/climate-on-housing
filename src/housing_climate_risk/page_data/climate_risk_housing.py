...
<head>
  ...
  <style>
    ...
    .county-count { justify-self: center; padding: 7px 14px; border: 3px solid var(--active-risk-color, var(--teal)); background: #fff; text-align: center; color: var(--ink); font-size: 17px; font-weight: 850; white-space: nowrap; }
    .risk-sequence-row { display: flex; flex-wrap: wrap; align-items: center; justify-content: center; gap: 10px 18px; margin: 4px auto 0; }
    .risk-sequence-legend { display: flex; flex-wrap: wrap; align-items: center; justify-content: center; gap: 5px 8px; }
    .risk-legend-button { position: relative; padding: 6px 9px 6px 25px; border: 1px solid var(--risk-color); border-radius: 999px; background: white; color: var(--muted); font-size: 10px; }
    .risk-legend-button::before { content: ""; position: absolute; left: 9px; top: 50%; width: 9px; height: 9px; border-radius: 2px; background: var(--risk-color); opacity: .42; transform: translateY(-50%); }
    .risk-legend-button.revealed { color: var(--ink); }
    .risk-legend-button.revealed::before { opacity: 1; }
    .risk-legend-button.active { border-color: var(--risk-color); background: var(--risk-color); color: white; box-shadow: 0 4px 11px color-mix(in srgb, var(--risk-color) 32%, transparent); }
    .risk-legend-button.active::before { background: white; opacity: 1; }
    .sequence-play-button, .feature-sequence-resume { display: inline-flex; align-items: center; justify-content: center; width: 32px; min-width: 32px; height: 30px; padding: 0; font-size: 12px; }
    .event-count-row { display: grid; grid-template-columns: minmax(0, 1.15fr) minmax(0, .85fr); align-items: center; gap: 16px; width: 100%; margin: 7px 0 0; }
    .event-count-row > .risk-sequence-row { width: 100%; margin: 0; }
    .event-overview { display: none; align-items: center; justify-content: center; min-height: min(54svh, 480px); }
    .event-overview.visible { display: flex; }
    .event-overview-chart { width: 100%; height: min(48svh, 430px); overflow: visible; }
    ...
    * { box-sizing: border-box; }
    ...
    .event-line { fill: none; stroke: #172026; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; }
    ...
    .takeaway { width: min(860px, calc(100% - 32px)); margin: 16px auto 0; border: 1px solid #a9cfc2; border-left: 6px solid var(--teal); border-radius: 0; background: #edf7f3; padding: 24px 28px; box-shadow: 0 16px 38px rgba(23,51,45,.20), 0 2px 8px rgba(23,51,45,.12); font-size: 18px; line-height: 1.5; font-weight: 750; }
    ...
    .tooltip { position: fixed; display: none; max-width: min(300px, calc(100vw - 16px)); max-height: calc(100svh - 16px); overflow: auto; white-space: pre-line; background: #172026; color: white; padding: 9px 10px; border-radius: 0; box-shadow: 0 8px 22px rgba(23,32,38,.28); font-size: 12px; line-height: 1.35; pointer-events: none; z-index: 1000; }
    ...
    .feature-sequence-resume { flex: 0 0 32px; margin: 0; }
    ...
    .feature-subgroup-control-stack { position: absolute; z-index: 100; left: 0; right: 5px; bottom: 0; display: none; align-items: center; justify-content: center; gap: 6px; isolation: isolate; pointer-events: auto; }
    #features .story-stage[data-story-state="feature-frame-2"] .feature-subgroup-control-stack,
    #features .story-stage[data-story-state="feature-frame-3"] .feature-subgroup-control-stack { display: flex; }
    #features .story-stage[data-story-state="feature-frame-2"] .feature-detail-stack,
    #features .story-stage[data-story-state="feature-frame-3"] .feature-detail-stack { padding-bottom: 38px; }
    #features .story-stage[data-story-state="feature-frame-2"] .feature-frame[data-frame="2"],
    #features .story-stage[data-story-state="feature-frame-3"] .feature-frame[data-frame="3"] { inset: 0 0 38px; height: auto; }
    .feature-subgroup-controls { display: flex; flex-wrap: nowrap; justify-content: center; gap: 4px; min-width: 0; pointer-events: auto; }
    .feature-subgroup-controls.visible { display: flex; }
    .feature-subgroup-control, .feature-subgroup-control * { cursor: pointer !important; user-select: none; }
    .feature-subgroup-control { position: relative; z-index: 102; width: auto; min-height: 29px; padding: 6px 7px 6px 22px; border: 1px solid var(--subgroup-color); border-radius: 999px; background: white; color: var(--muted); text-align: left; font-size: 9px; cursor: pointer !important; pointer-events: auto; touch-action: manipulation; user-select: none; white-space: nowrap; }
    .feature-subgroup-control::before { content: ""; position: absolute; left: 8px; top: 50%; width: 9px; height: 9px; border-radius: 2px; background: var(--subgroup-color); opacity: .55; transform: translateY(-50%); cursor: pointer; pointer-events: none; }
    ...
    .playbook-selected-layout { display: grid; grid-template-columns: minmax(0, 1.8fr) minmax(250px, .8fr); grid-template-rows: auto minmax(0, 1fr); gap: 12px 14px; height: 100%; min-height: 0; align-items: stretch; }
    .playbook-profile-panel { display: none; min-width: 0; padding: 12px; border: 1px solid var(--line); background: #f8fbf9; overflow: hidden; }
    .playbook-profile-map-pane, .playbook-history-pane, .playbook-events-pane, .playbook-performance-pane, .playbook-commentary-pane { display: none; min-width: 0; min-height: 0; }
    .playbook-profile-map-pane { position: relative; overflow: hidden; }
    .playbook-profile-map-pane .chart { height: 100%; }
    .playbook-history-pane { flex-direction: column; }
    .playbook-history-pane .chart { flex: 1 1 auto; min-height: 0; height: 100%; }
    .playbook-events-pane, .playbook-performance-pane, .playbook-commentary-pane { overflow: hidden; }
    .playbook-events-pane { flex-direction: column; }
    ...
    .playbook-back-button { position: absolute; z-index: 12; left: 0; top: 0; border-radius: 0; padding: 5px 8px; font-size: 10px; line-height: 1.15; }
    .playbook-selected-frame { padding-top: 34px; }
    ...
    .story-stage .takeaway.story-outgoing-takeaway { position: absolute; z-index: 8; left: 50%; right: auto; top: auto; bottom: var(--takeaway-bottom, 8px); width: min(820px, calc(100% - 40px)); max-width: 820px; max-height: none; margin: 0; padding: 10px 14px; opacity: 1; overflow: hidden; transform: translateX(-50%); font-size: clamp(13px, 1.15vw, 17px); line-height: 1.3; box-shadow: 0 16px 38px rgba(23,51,45,.22), 0 2px 8px rgba(23,51,45,.14); }
    ...
    #playbook .story-stage[data-story-state="history-map"] .playbook-selected-frame,
    #playbook .story-stage[data-story-state="history-events"] .playbook-selected-frame,
    #playbook .story-stage[data-story-state="history-compare"] .playbook-selected-frame,
    #playbook .story-stage[data-story-state="history-outlook"] .playbook-selected-frame { opacity: 1; transform: translateY(0); pointer-events: auto; }
    #playbook .story-stage[data-story-state^="history-"] .playbook-history-pane { display: flex; grid-column: 1; grid-row: 1 / 3; }
    #playbook .story-stage[data-story-state="history-map"] .playbook-profile-map-pane { display: block; grid-column: 2; grid-row: 1 / 3; }
    #playbook .story-stage[data-story-state="history-events"] .playbook-profile-panel,
    ...
    #playbook .story-stage[data-story-state="history-outlook"] .playbook-history-pane { display: none; }
    ...
    #playbook .story-stage[data-story-state="history-outlook"] .playbook-commentary-pane { display: grid; grid-template-rows: auto minmax(0, 1fr) auto; gap: 12px; grid-column: 1; grid-row: 2; overflow: hidden; padding: 0 3px 8px; }
    #playbook .story-stage[data-story-state="history-outlook"] #playbook-warning-intro { grid-row: 1; }
    #playbook .story-stage[data-story-state="history-outlook"] .playbook-commentary { grid-row: 2; min-height: 0; height: auto; margin: 0; padding: 0; border: 0; background: transparent; box-shadow: none; overflow-y: auto; }
    #playbook .story-stage[data-story-state="history-outlook"] .playbook-warning-grid { min-height: 100%; align-content: stretch; align-items: stretch; }
    #playbook .story-stage[data-story-state="history-outlook"] #playbook-warning-takeaway { grid-row: 3; margin-top: 0; align-self: end; }
    ...
    .playbook-performance-status strong { font-size: 17px; color: var(--teal); }
    ...
    .playbook-warning-grid { display: flex; flex-wrap: wrap; justify-content: space-between; gap: 7px; margin: 0; }
    .playbook-warning-row { flex: 1 1 calc((100% - 21px) / 4); min-width: 0; box-sizing: border-box; display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 10px; align-items: center; min-height: 72px; padding: clamp(10px, 2vh, 24px) clamp(10px, 1.5vw, 22px); border: 1px solid var(--line); background: white; font-size: clamp(12px, 1.1vw, 16px); line-height: 1.4; }
    ...
    @media (max-width: 900px) {
      ...
      .playbook-selected-layout { grid-template-columns: minmax(0, 1.45fr) minmax(210px, .8fr); gap: 8px; }
    ...
    .playbook-scorecard { display: grid; grid-template-columns: minmax(0, 1fr) auto minmax(0, 1fr) auto minmax(0, 1fr); }
    .playbook-scorecard-title, .playbook-score-result { grid-column: 1 / -1; min-width: 0; }
    .playbook-scorecard-title { font-size: 20px; line-height: 1.25; white-space: nowrap; }
    .playbook-scorecard-part { min-width: 0; }
    .playbook-scorecard-plus { align-self: center; font-size: 26px; font-weight: 800; color: var(--forest); }
    #t-hero-h1, #t-hero-dek { text-align: center; margin-left: auto; margin-right: auto; }
    .hero > * { width: 100%; }
    .story-stage .takeaway:not(.story-active-takeaway) { visibility: hidden; opacity: 0 !important; pointer-events: none; }
    .story-stage .takeaway.story-active-takeaway { visibility: visible; }
    .feature-subgroup-summary-row { display: flex; flex-direction: column; align-items: stretch; gap: 7px; }
    .feature-peer-percentage { font-size: 12px; line-height: 1.45; }
    #playbook .story-stage[data-story-state="history-outlook"] .playbook-commentary-pane { grid-template-rows: auto minmax(0, 1fr) auto; gap: clamp(8px, 1.3vh, 16px); overflow: hidden; }
    #playbook .story-stage[data-story-state="history-outlook"] .playbook-commentary { padding: 10px; border: 1px solid #bacdc3; border-radius: 8px; background: #edf3ef; overflow-y: auto; }
    .playbook-warning-intro { font-size: clamp(17px, 1.6vw, 21px); font-weight: 800; }
    .playbook-factor-columns { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); gap: 12px; flex: 1; min-height: 0; }
    .playbook-factor-column { display: flex; flex-direction: column; min-width: 0; min-height: 0; overflow: hidden; padding: 10px; border: 1px solid #a4cdb4; border-radius: 8px; background: #e4f2e8; }
    .playbook-factor-column.negative { background: #f9e8e7; border-color: #dfb1ae; }
    .playbook-factor-column h4 { margin: 0 0 8px; font-size: 14px; padding-bottom: 6px; border-bottom: 3px solid #11794f; }
    .playbook-factor-column.negative h4 { border-bottom-color: #be302e; }
    .playbook-factor-list { display: flex; flex-direction: column; flex: 1; min-height: 0; gap: 8px; overflow-y: auto; overscroll-behavior: contain; scrollbar-gutter: stable; }
    .playbook-factor-list .playbook-warning-row { flex: 1 0 auto; align-self: stretch; padding: clamp(8px, 1vh, 14px); }
    .playbook-factor-name { display: flex; align-items: center; gap: 6px; min-width: 0; }
    .playbook-factor-list .playbook-warning-row, .playbook-factor-other .playbook-warning-row { grid-template-columns: minmax(0, 1fr) auto auto; }
    .playbook-factor-position { color: var(--muted); font-size: 12px; white-space: nowrap; text-align: right; }
    .playbook-factor-other { margin-top: 10px; max-height: 25%; overflow-y: auto; flex-shrink: 0; }
    .playbook-factor-empty { color: var(--muted); font-size: 16px; flex: 1; margin: 0; display: flex; align-items: center; justify-content: center; text-align: center; }
    .playbook-scorecard-part, .playbook-score-result { background: #f4f5f1; color: var(--ink); border-color: #cbd5cc; }
    .playbook-scorecard-value > span:first-child { font-size: clamp(19px, 2.1vw, 30px); font-weight: 800; line-height: 1.15; }
    .playbook-scorecard .playbook-arrow-up { color: #11794f; }
    .playbook-scorecard .playbook-arrow-down { color: #be302e; }
    .playbook-score-result { white-space: normal; overflow: visible; display: flex; flex-direction: column; min-height: 105px; }
    .playbook-score-result b { font-size: clamp(21px, 2vw, 28px); }
    .score-risk-low { color: #11794f; } .score-risk-moderate { color: #976200; } .score-risk-high { color: #be302e; }
    .playbook-score-takeaway { text-align: center; margin: auto 0 0; padding-top: 14px; font-size: 15px; }
    .playbook-score-result a { color: #126354; }
    @media (max-width: 700px) { .playbook-scorecard-value > span:first-child { font-size: 17px; } .playbook-factor-columns { gap: 8px; } }
    #playbook-warning-takeaway { background: var(--forest); color: white; border-color: var(--forest); }
    .playbook-scorecard { text-align: center; }
    .playbook-scorecard-plus { color: white; }
    .playbook-scorecard-value { justify-content: center; gap: 8px; }
    .playbook-scorecard-value > span:first-child { font-size: 16px; white-space: nowrap; min-width: 0; }
    .playbook-scorecard-value .playbook-score-arrow { flex: 0 0 auto; margin: 0; }
    .playbook-score-result { text-align: center; }
    .feature-subgroup-summary-row { gap: 3px; padding: 6px 8px; line-height: 1.25; }
    .feature-subgroup-summary-row strong, .feature-peer-percentage { margin: 0; line-height: 1.25; }
    .feature-subgroup-summary-rows { gap: 5px; }
    .feature-subgroup-summary-rows { flex: 1; grid-auto-rows: minmax(min-content, 1fr); align-content: stretch; }
    .feature-subgroup-summary-row { justify-content: space-evenly; padding: 8px 12px; }
    .feature-subgroup-factor-name { display: flex; align-items: center; gap: 6px; }
    .feature-peer-percentage { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
    .feature-peer-percentage .feature-percentage-value { font-size: clamp(22px, 2vw, 30px); font-weight: 800; color: var(--forest); flex-shrink: 0; }
    #playbook .story-stage[data-story-state="history-outlook"] > .panel { padding-bottom: 14px; }
    #playbook .story-stage[data-story-state="history-outlook"] > .panel > .sources { position: static; flex: 0 0 auto; margin-top: 5px; padding-top: 5px; }
    #playbook .story-stage:not([data-story-state="history-outlook"]) #playbook-warning-intro { display: none; }
    #playbook .story-stage[data-story-state="history-outlook"] #playbook-warning-intro { position: absolute; top: 0; left: 190px; right: 190px; text-align: center; line-height: 1.2; }
    #playbook .story-stage[data-story-state="history-outlook"] .playbook-commentary-pane { grid-template-rows: minmax(0, 1fr) auto; padding: 0; }
    #playbook .story-stage[data-story-state="history-outlook"] .playbook-commentary { grid-row: 1; padding: 0; border: 0; border-radius: 0; background: transparent; box-shadow: none; display: flex; flex-direction: column; overflow: hidden; }
    #playbook .story-stage[data-story-state="history-outlook"] #playbook-warning-takeaway { grid-row: 2; padding: 9px 12px; }
    .playbook-factor-column h4 { flex-shrink: 0; text-align: center; font-size: clamp(15px, 1.2vw, 18px); }
    .playbook-factor-list .playbook-warning-row { font-size: clamp(12px, 1vw, 15px); }
    .playbook-scorecard { width: 92%; margin: 0 auto; }
    .playbook-score-result { min-height: 0; padding: 7px 10px; }
    .playbook-score-takeaway { padding-top: 5px; }
    .playbook-score-result a { font-weight: 800; }
    .playbook-scorecard-value.favorable > span:first-child { color: #11794f; }
    .playbook-scorecard-value.unfavorable > span:first-child { color: #be302e; }
    #playbook .panel > .sources { margin-top: 5px; padding-top: 5px; }
    @media (max-width: 700px) { #playbook .story-stage[data-story-state="history-outlook"] #playbook-warning-intro { left: 160px; right: 0; font-size: 15px; } .playbook-scorecard { width: 100%; } }
  </style>
</head>
<body>
<main>
    ...
    <div class="panel" style="margin-top:16px;">
      ...
      <div class="viz-grid pricing-viz-grid">
        <div class="rating-line-pane">
          ...
          <div class="risk-sequence-row">
            <div id="rating-risk-legend" class="risk-sequence-legend" aria-label="NRI risk rating animation frames"></div>
            <button id="rating-play-button" class="sequence-play-button" type="button"></button>
          </div>
        </div>
  ...
  <section class="slide" id="events">
    ...
    <div class="panel" style="margin-top:12px;">
      ...
      <div id="event-detail-grid" class="viz-grid timeseries-grid">
      ...
      <div id="event-sequence-row" class="event-count-row">
        <div class="risk-sequence-row">
          <div id="event-risk-legend" class="risk-sequence-legend" aria-label="Event risk rating animation frames"></div>
          <button id="event-play-button" class="sequence-play-button" type="button"></button>
        </div>
      ...
      <div class="playbook-frame-stack">
        ...
        <div class="playbook-frame playbook-selected-frame">
          ...
          <div class="playbook-selected-layout">
            ...
            <aside class="playbook-performance-pane">
